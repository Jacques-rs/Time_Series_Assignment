...
Figures \ref{reers} \& \ref{detreers} showcases each variable as they as they 
were before their natural logarithm transformation.


```{r fig.cap="The South Africa Real Exchange Rate \\label{reers}"}
...
will be included as an alternative specification.

```{r fig.width=7, fig.cap="Determinants of the Real Exchange Rate for South Africa \\label{detreers}"}
plot_endog2(df = full_df)
```

Figure \ref{detreers} above suggests that there is a possible structural break
in the time series around the year 1985. This is mostly evident in the behaviour
of the variable `NFAOFPY`, the Net Foreign Assets proxy. The openness indicator,
Relative Real Interest Rate, Relative Real GDP, and Real Commodity prices 
slightly correspond to this theory as well. A test for this is therefore 
necessary.
...
```{r fig.cap="The Joint plot of the Model Varibales (Logged where necessary) \\label{joint}"}
...
declining trend, after which the co-movement seems more evident.
...
order (i.e. $REER \sim I(1)$), and includes a constant term.
...
# 1. Estimating the Unrestricted VAR in Levels and Checking Adequacy
